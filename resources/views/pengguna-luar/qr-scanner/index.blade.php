...
function showBookPreview(qrCode) {
    ...
    fetch('{{ route("pengguna-luar.qr.preview") }}', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            content.innerHTML = `
                ...
                <div class="book-preview-item">
                    <label>Sisa Kuota Anda</label>
                    <p><span class="badge bg-info">${book.sisa_kuota} dari 2 buku</span></p>
                </div>
                ${book.info ? `
                    <div class="alert alert-info mt-3 mb-0">
                        <small><i class="bi bi-info-circle me-1"></i> ${book.info}</small>
                    </div>
                ` : ''}
            `;
            
            if (book.stok > 0 && book.sisa_kuota > 0) {
                confirmBtn.classList.remove('d-none');
            } else if (book.sisa_kuota <= 0) {
                content.innerHTML += `
                    <div class="alert alert-warning mt-3">
                        <i class="bi bi-exclamation-triangle me-2"></i> 
                        Anda sudah mencapai batas maksimal peminjaman (2 buku). Harap kembalikan buku terlebih dahulu.
                    </div>
                `;
            }
        } else {
...
function confirmBorrow() {
    ...
    fetch('{{ route("pengguna-luar.qr.process") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        if (data.success) {
            content.innerHTML = `
                <div class="alert alert-success text-center">
                    ...
                    <small class="text-muted">Durasi: ${data.data.durasi_hari} hari</small><br>
                    ${data.data.sisa_kuota !== undefined ? `<small class="text-info">Sisa kuota: ${data.data.sisa_kuota} buku</small>` : ''}
                </div>
            `;
            
            setTimeout(() => {
                modal.hide();
                window.location.href = '{{ route("pengguna-luar.peminjaman.riwayat") }}';
            }, 3000);
        } else {
            content.innerHTML = `
                <div class="alert alert-danger">
